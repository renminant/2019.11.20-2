...
<body>
	...
	<div class="menu">
		<form method="post" action="/ren/userList.action">
		<table>
			<tbody>
				<tr>
			...
			<table class="list">
				...
				<c:forEach var="user" items="${pageinfo.list}">
					<tr>
						...
						<td><a href="/ren/selectUserViewByid.action?user_id=${user.user_id}">
							<c:out value="${user.name}"></c:out></a>
						</td>
			           <td>
						<c:out value="${user.loginName}"></c:out></td>
						<td>
		...
		$(function () {
			...
			a.on("click",function () {
				for (var i=0;i<a.length;i++){
					var user_id = $(this)[i].parentNode.parentNode.children[0].innerHTML;
					console.log(user_id);
					location.href = "<%=request.getContextPath()%>/selectUserByid.action?user_id="+user_id;
				}
			})
		})
	</script>
